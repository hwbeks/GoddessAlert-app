          ...
          <div style={weeklyRating ? css.cardAccent : css.card}>
            ...
            {!weeklyRating ? (
              <div style={{ display: "flex", gap: 8 }}>
                {[{ label: "😬", sub: "Poor", val: "poor", pts: -8 }, { label: "😐", sub: "OK", val: "ok", pts: 0 }, { label: "😊", sub: "Good", val: "good", pts: 8 }, { label: "🔥", sub: "Great", val: "great", pts: 16 }].map((opt) => (
                  <button key={opt.val} onClick={async () => {
                    setWeeklyRating(opt.val); setScore((s) => Math.min(100, Math.max(0, s + opt.pts)));
                    const { data: { user } } = await supabase.auth.getUser();
                    if (user) { const sv = opt.val === "great" ? 4 : opt.val === "good" ? 3 : opt.val === "ok" ? 2 : 1; await supabase.from("health_scores").insert({ user_id: user.id, score: sv }); }
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div>
                  <div style={{ fontSize: 15, color: T.text }}>{weeklyRating === "poor" && "😬 You rated: Poor"}{weeklyRating === "ok" && "😐 You rated: OK"}{weeklyRating === "good" && "😊 You rated: Good"}{weeklyRating === "great" && "🔥 You rated: Great"}</div>
                  <div style={{ fontSize: 12, color: T.muted, marginTop: 4 }}>{weeklyRating === "poor" && "Small steps. Start with today's tip."}{weeklyRating === "ok" && "Solid start. Push a little harder this week."}{weeklyRating === "good" && "She notices. Keep the momentum going."}{weeklyRating === "great" && "That's the standard. Don't let it slip."}</div>
                </div>
            ...
            <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 16, padding: "28px 18px", marginBottom: 10, textAlign: "center" }}>
              ...
              <div style={{ display: "inline-flex", gap: 8, alignItems: "center", background: T.accentSoft, border: `1px solid ${T.accent}44`, borderRadius: 12, padding: "12px 20px" }}>
                {[{ val: "18", label: "hrs" }, { val: ":", label: null }, { val: "24", label: "min" }, { val: ":", label: null }, { val: "07", label: "sec" }].map((seg, i) =>
                  seg.label === null ? <div key={i} style={{ fontSize: 22, color: T.accent, fontWeight: "bold", paddingBottom: 14 }}>:</div> :
            ...
            <div key={ev.id} style={{ ...css.cardAccent, marginTop: 8 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <div><div style={{ fontSize: 15, fontWeight: "bold" }}>{ev.name}</div><div style={{ fontSize: 12, color: T.muted, marginTop: 2 }}>Don't wait — plan something now</div></div>
                </div>
                <div style={{ textAlign: "center" }}>
          ...
          <div style={{ fontSize: 11, color: T.muted, textAlign: "center", marginTop: 12, marginBottom: 8, lineHeight: 1.6 }}>
            ...
            {showTheCode ? (
              <div style={{ padding: "32px 8px 16px", textAlign: "center" }}>
                ...
                {theCodeContent.map(([num, title, body]) => (<div key={num} style={{ marginBottom: 24, textAlign: "left" }}><div style={{ fontSize: 10, color: T.muted, letterSpacing: 2 }}>{num}</div><div style={{ fontSize: 13, color: T.accent, fontStyle: "italic", marginBottom: 6 }}>{title}</div><div style={{ fontSize: 12, color: T.muted, lineHeight: 1.6 }}>{body}</div></div>))}
              </div>
            ) : (
